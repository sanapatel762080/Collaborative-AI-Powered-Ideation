...
// // // In-memory chat storage per project (optional, can remove if using DB only)
// // const chatsCache = {};
...
// //     // Send welcome and system messages
// //     socket.emit('system', `Welcome to project ${projectId}, ${user}`);
// //     socket.to(projectId).emit('system', `${user} joined the project`);

// //     // Optional: Send previous messages from DB
// //     const previousMessages = await Message.find({ projectId }).sort({ createdAt: 1 }).lean();
...
// //       const msg = await Message.create({ projectId, sender, text });
...
// //       socket.to(currentProjectId).emit('system', `${currentUser} left the project`);
// //     }
// //   });
// // });
...
//     // Send welcome and system messages
//     socket.emit('system', `Welcome ${user} to project ${projectId}`);
//     socket.to(projectId).emit('system', `${user} joined the project`);
//   });
